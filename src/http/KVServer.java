...
public class KVServer {
    ...
    private void save(HttpExchange httpExchange) {
        try {
            ...
            System.out.println("Ошибка в ходе отправки статуса запроса метода save() \n" + e.getMessage());
            try {
                httpExchange.sendResponseHeaders(500, 0);
            } catch (IOException ex) {
                System.out.println("Ошибка отправки данных метода save()\n" + e.getMessage());
            }
        } finally {
            httpExchange.close();
        }
    ...
    private void register(HttpExchange httpExchange) {
        try {
            ...
            try {
                ...
                System.out.println("Ошибка отправки данных  метода register() \n" + e.getMessage());
            }
        } finally {
            httpExchange.close();
        }
